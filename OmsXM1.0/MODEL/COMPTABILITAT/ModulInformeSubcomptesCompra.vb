...
'End Module
'' fi copia de seguretat 17/04/2023

'''

'' copia edicio 17/04/2023 xmartimoret
'Imports EXC = Microsoft.Office.Interop.Excel
'Module ModulInformeSubcomptesCompra
'    Private subcomptes As List(Of SubcompteGrup)
'    Private dataInicial As Date
...
'    ''' <summary>
'    ''' setData. 
'    ''' 1. Agafar les dades de compres
'    ''' 2. per cada subcompte de compres. Cal saber  els proveidors i el consum per proveidor. 
'    ''' 3. per cada subcompte de compra cal saber el saldo 
'    ''' 4. per cada proveidor cal esbrinar en cada assentament quin consum te 
'    ''' </summary>
'    ''' <param name="c"></param>
'    Private Sub setData(c As Contaplus)
'        Dim wb As EXC.Workbook, ws As EXC.Worksheet, f As frmAvis
...
'                If rc.State = 1 Then rc.Close()
'                f.setData(xls(i, 1).value & "-" & xls(i, 2).value, i)
'                ' agafar tots els assentaments per cada sbucompte
'                rc1.Open("SELECT SUM(EURODEBE) AS debe ,SUM (EUROHABER) as haber FROM " & taulaDiario & "  WHERE SUBCTA ='" & xls(i, 1).value & "' and CONCEPTO not like '%pagament%'", cn)
'                If Not rc1.EOF Then
'                    If rc1("debe").Value + rc1("haber").Value > 0 Then
'                        sCompra = New SubcompteGrup
'                        sCompra.codiSubcompte = rc("sCompra").Value
'                        sCompra.idSubcompte = ModelSubcompte.getId(rc("sCompra").Value)
'                        sCompra.valorDeure = rc1("debe").Value
...
'                        xls(i, 4).value = rc1("haber").Value
'                        xls(i, 5).value = xls(i, 3).value - xls(i, 4).value
'                        'aqui cal tornar  a llistar els assentaments del subcompte escollit 
'                        rc1.Close()
'                        rc1.Open("SELECT asien FROM " & taulaDiario & "  WHERE SUBCTA ='" & xls(i, 1).value & "' and CONCEPTO not like '%pagament%'", cn)
'                        While Not rc.EOF
'                            rc.Open("select subcta,fecha,concepto,eurodebe,eurohaber,departa,clave,documento  FROM " & taulaDiario & "  WHERE ASIEN = " & rc1("asien").Value, cn)
'                            a = New Assentament
'                            a.deure = Math.Round(rc("EURODEBE").Value * 100, 0)
'                            a.haver = Math.Round(rc("EUROHABER").Value * 100, 0)
'                            a.numero = rc("ASIEN").Value
...
'                            a.concepte = rc("concepte").Value
'                            a.clave = rc("clave").Value
'                            a.departamentAssentament = rc("departa").Value
'                            a.document = rc("documento").Value
...
'End Module
''fi copia edicio 17/04/2023 xmartimoret
