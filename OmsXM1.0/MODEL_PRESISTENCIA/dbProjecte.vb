...
Module dbProjecte
    ...
    Private Function insertSQL(obj As Projecte) As Integer
        ...
        sc = New SqlCommand(" INSERT INTO " & getTable() & " " &
                                " (" & ID & ", " & ID_EMPRESA & ", " & CODI & ", " & NOM & ", " & NOTES & ")" &
                                " VALUES(@id,@idEmpresa,@
                                codi,@nom,@notes)", DBCONNECT.getConnection)
    ...
    Private Function updateSQL(obj As Projecte) As Integer
        ...
        sc = New SqlCommand("UPDATE " & getTable() & " " &
                                " SET " & ID_EMPRESA & "=@idEmpresa," & CODI & "=@codi," & NOM & " =@nom, " & NOTES & "=@notes " &
    ...
    Private Function getObjectsSQL() As List(Of Projecte)
        ...
        While sdr.Read()
            dades.Add(New Projecte(sdr(ID), sdr(ID_EMPRESA), Trim(sdr(CODI)), Trim(sdr(CODI)), Trim(sdr(NOM)), "", ModelEmpresa.getObject(sdr(ID_EMPRESA)).ToString))
